(function() {
    ...
    async function loadAllContent() {
        const contentFiles = [
            ...
            'evidence-scientific-methods.txt', 'evidence-scientific-sources.txt', 'evidence-scientific-appraisal.txt', 'documentation-process.txt', 'keywords.md', 'keywords.html',
        ...
        for (let filename of contentFiles) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Find the target container by data-filename or old onclick fallback
                    const targetDiv = document.querySelector(`[data-filename="${filename}"]`) || document.querySelector(`[onclick*="${filename}"]`);
                    if (targetDiv) {
                        ...
                        if (pre) {
                            try {
                                ...
                                // If processed content contains HTML wrappers, use innerHTML, otherwise plain text
                                if (content.trim().startsWith('<div') || content.includes('<h3') || content.includes('<table') ) {
                                    pre.innerHTML = content;
                                } else {
                                    pre.textContent = content.replace(/<br>/g, '\n');
                                }

                                // CLEAN UP small placeholder hints but keep View/Edit intact
    ...
    function processMarkdown(content) {
        ...
        content = content
            .replace(/###HEADER1###(.*?)###HEADER1###/g, '<h3 style="color: var(--accent-blue); margin-top: 20px; margin-bottom: 10px; font-weight: 600;">$1</h3>')
            .replace(/###HEADER2###(.*?)###HEADER2###/g, '<h4 style="color: var(--accent-red); margin-top: 15px; margin-bottom: 8px; font-weight: 600;">$1</h4>')
